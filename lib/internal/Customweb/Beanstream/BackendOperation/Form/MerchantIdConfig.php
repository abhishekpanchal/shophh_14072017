 ...
 * @BackendForm
 */
final class Customweb_Beanstream_BackendOperation_Form_MerchantIdConfig extends Customweb_Payment_BackendOperation_Form_Abstract {
	const CONIFGURED_LIST_KEY = 'configuredCurrencies';
	const STORAGE_SPACE_KEY = 'Beanstream_MIDs';
	...
	public function getTitle(){
		return Customweb_I18n_Translation::__("Merchant ID / Currency Config");
	}
	...
	private function listExisting(){
		...
		foreach ($this->getConfiguredCurrencies() as $currency) {
			...
			$group->setTitle($currencies[$currency]['code'] . ' (' . $currencies[$currency]['name'] . ')')->setId($currency)->setMachineName(
					$currency);
			
			$liveControl = new Customweb_Form_Control_TextInput($currency . '_live', $this->getSettingValue($currency . '_live'));
			$liveElement = new Customweb_Form_Element(Customweb_I18n_Translation::__('Live Merchant ID'), $liveControl);
			
			$testControl = new Customweb_Form_Control_TextInput($currency . '_test', $this->getSettingValue($currency . '_test'));
			$testElement = new Customweb_Form_Element(Customweb_I18n_Translation::__('Test Merchant ID'), $testControl);
