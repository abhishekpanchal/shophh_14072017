...
class Upload extends \Altima\Lookbookslider\Controller\Adminhtml\Slide {
    ...
    $options = null, $initialize = true, $error_messages = null) {
        
        /*   Create resize image */
        $slide_options = $this->get_post_param('slide');
        //$new_url = $this->_objectManager->create('Altima\Lookbookslider\Helper\Data')
        //                ->getResizedUrl($content['files'][0]->url, $slide_options['image_width'], $slide_options['image_height']);
        //$content['files'][0]->url = $new_url;
        /**                      **/
        
        $uploadDir = $this->getMediaBaseUrl() . 'lookbookslider/';
        // $om = \Magento\Framework\App\ObjectManager::getInstance(); $dir = $om->get('Magento\Framework\Filesystem');  
        // $mediadir=$dir->getDirectoryWrite(\Magento\Framework\App\Filesystem\DirectoryList::MEDIA); 
        // $path = $mediadir->create('lookbookslider');
        $path = $this->getAbsoluteMediaPath('') . 'lookbookslider/';

        //$path = $mediadir . 'lookbookslider';
        //return $path;
        $this->response = array();
        $this->options = array(
            ...
            'image_versions' => array(
                 ...
                 */
                  ...
                  $slide_options['image_width'].'X'.$slide_options['image_height'] => array(
                  'max_width' => $slide_options['image_width'],
                  'max_height' => $slide_options['image_height']
                  ),
...
/* Create resize image */
        $slide_options = $this->get_post_param('slide');
        $path = $this->getAbsoluteMediaPath('') . 'lookbookslider/'.$file_name;
        return $this->_objectManager->create('Altima\Lookbookslider\Helper\Data')
                       ->getResizedUrl($file_name, $slide_options['image_width'], $slide_options['image_height']);
       
/** end create resize **/

        
        if ($this->options['image_library'] === 2) {
            $scaled_image =$this->imagemagick_create_scaled_image($file_name, $version, $options);
            return $scaled_image;
        }
        if ($this->options['image_library'] && extension_loaded('imagick')) {
            $scaled_image = $this->imagick_create_scaled_image($file_name, $version, $options);
            return $scaled_image;
        }
        $scaled_image = $this->gd_create_scaled_image($file_name, $version, $options);
        return $scaled_image;
    }
    ...
    protected function handle_image_file($file_path, $file) {
        ...
        foreach ($this->options['image_versions'] as $version => $options) {
            /*******/
            $scaled = $this->create_scaled_image($file->name, $version, $options);
            /*******/
            //if ($this->create_scaled_image($file->name, $version, $options)) {
            if ($scaled) {
                if (!empty($version)) {
                    /*$file->{'Url'.$version} = $this->get_download_url(
                            $file->name, $version
                    );*/
                    $file->{'Url'.$version} = $scaled;
                } else {
    ...
    protected function get_upload_data($id) {
      $post_files = $this->getRequest()->getFiles($id);
      //return @$_FILES[$id];
      return $post_files[0];
    }
    ...
    public function generate_response($content, $print_response = true) {
        ...
        /* Create resize image  
        $slide_options = $this->get_post_param('slide');
        $path = $this->getAbsoluteMediaPath('') . 'lookbookslider/'.$content['files'][0]->name;
        $new_url = $this->_objectManager->create('Altima\Lookbookslider\Helper\Data')
                        ->getResizedUrl($path, $slide_options['image_width'], $slide_options['image_height']);
        $content['files'][0]->url = $new_url;
