...
abstract class AbstractStatement extends AbstractModel implements StatementInterface
{
    ...
    protected function _formatRefunds()
    {
        ...
        foreach ($this->_refunds as &$refund) {
            if (!empty($refund['amounts'])) $this->_hlp->formatAmounts($refund, $refund['amounts'], 'merge');
        }
        unset($refund);
        return $this;
    }
    ...
    protected function _formatOrders()
    {
        $this->initTotals();
        foreach ($this->_orders as &$order) {
            if (!empty($order['amounts'])) $this->_hlp->formatAmounts($order, $order['amounts'], 'merge');
        }
        unset($order);
        return $this;
    }
    ...
    public function processPos($pos, $subtotalBase)
    {
        ...
        if ($poItemsToLoad) {
            ...
            $fields[] = 'base_cost';
            $poItems->getSelect()
                ->join(array('i'=>$poItems->getTable('sales_order_item')), 'i.item_id=order_item_id', $fields)
