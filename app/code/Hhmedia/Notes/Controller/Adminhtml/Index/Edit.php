...
class Edit extends \Magento\Backend\App\Action
{
    ...
    public function execute()
    {
        ...
        if ($id) {
            $model->load($id);
            if (!$model->getId()) {
                $this->messageManager->addError(__('This notes no longer exists.'));
        ...
        $this->_coreRegistry->register('notes', $model);

        // 5. Build edit form
		/** @var \Magento\Backend\Model\View\Result\Page $resultPage */
        $resultPage = $this->_initAction();
        $resultPage->addBreadcrumb(
            $id ? __('Edit Notes') : __('New Notes'),
            $id ? __('Edit Notes') : __('New Notes')
        );
        $resultPage->getConfig()->getTitle()->prepend(__('Notes'));
        $resultPage->getConfig()->getTitle()
            ->prepend($model->getId() ? $model->getTitle() : __('New Notes'));
        return $resultPage;
    }
}
