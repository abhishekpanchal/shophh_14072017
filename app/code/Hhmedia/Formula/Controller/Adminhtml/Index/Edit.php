...
class Edit extends \Magento\Backend\App\Action
{
    ...
    {
        return $this->_authorization->isAllowed('Hhmedia_Formula::save');
    }
    ...
    public function execute()
    {
        ...
        $model = $this->_objectManager->create('Hhmedia\Formula\Model\Formula');

        // 2. Initial checking
        if ($id) {
            $model->load($id);
            if (!$model->getId()) {
                $this->messageManager->addError(__('This formula no longer exists.'));
        ...
        $this->_coreRegistry->register('formula', $model);
        ...
        $resultPage->getConfig()->getTitle()
            ->prepend($model->getId() ? $model->getTitle() : __('New Formula'));
        return $resultPage;
    }
}
