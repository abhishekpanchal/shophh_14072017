...
class Edit extends \Magento\Backend\App\Action
{
    ...
    public function execute()
    {
        ...
        if ($id) {
            $model->load($id);
            if (!$model->getId()) {
                $this->messageManager->addError(__('This editor no longer exists.'));
        ...
        $this->_coreRegistry->register('editor', $model);

        // 5. Build edit form
		/** @var \Magento\Backend\Model\View\Result\Page $resultPage */
        $resultPage = $this->_initAction();
        $resultPage->addBreadcrumb(
            $id ? __('Edit Editor') : __('New Editor'),
            $id ? __('Edit Editor') : __('New Editor')
        );
        $resultPage->getConfig()->getTitle()->prepend(__('Editor'));
        $resultPage->getConfig()->getTitle()
            ->prepend($model->getId() ? $model->getTitle() : __('New Editor'));
        return $resultPage;
    }
}
