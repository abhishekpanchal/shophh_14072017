...
namespace Customweb\BeanstreamCw\Plugin\Sales\Controller\Adminhtml\Order\Create;
...
class Save
{
	...
	{
		$this->_coreRegistry->register('beanstreamcwcheckout_moto', true);
		$result = $proceed();
		if ($this->_messageManager->getMessages()->getLastAddedMessage()->getType() == 'success') {
			$this->_messageManager->getMessages()->clear();
			$order = $this->_coreRegistry->registry('beanstreamcw_checkout_last_order');
			if ($order instanceof \Magento\Sales\Model\Order) {
				if ($order->getPayment()->getMethodInstance() instanceof \Customweb\BeanstreamCw\Model\Payment\Method\AbstractMethod) {
					return $this->_redirectResultFactory->create()->setPath('beanstreamcw/checkout/payment', ['order_id' => $order->getId()]);
				}
			}
		}
		return $result;
