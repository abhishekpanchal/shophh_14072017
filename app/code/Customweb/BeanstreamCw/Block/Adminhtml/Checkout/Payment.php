...
namespace Customweb\BeanstreamCw\Block\Adminhtml\Checkout;
...
class Payment extends \Magento\Backend\Block\Widget\Form\Container
{
	 ...
	 * @var string
	 */
	protected $_blockGroup = 'Customweb_BeanstreamCw';
	...
	protected function _construct()
	{
		...
		$this->setId('beanstreamcw_checkout_payment');

		$this->buttonList->update('save', 'label', __('Submit Order'));
		$this->buttonList->update('save', 'class', 'primary');
		$this->buttonList->update('save', 'id', 'beanstreamcw-payment-submit-button');
		...
		$this->buttonList->add(
			...
			[
				'label' => __('Cancel'),
				'class' => 'cancel',
				'id' => 'beanstreamcw-payment-cancel-button'
			]
		);
	}
	...
	public function getTransaction()
	{
		if (!($this->transaction instanceof \Customweb\BeanstreamCw\Model\Authorization\Transaction)) {
			...
			if (!($transaction instanceof \Customweb\BeanstreamCw\Model\Authorization\Transaction) || !$transaction->getId()) {
