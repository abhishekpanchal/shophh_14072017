...
define([
    ...
    $.widget('mage.SwatchRenderer', {
        ...
        // Gets the selected options data, replaces content on page with it
        _UpdateProductData: function () {
            ...
            var selectedProductData = $widget.options.jsonConfig.product_data[productId];
            if (typeof selectedProductData == 'undefined') {
                console.log('No product data....');
                return false;
            }

            // For each attribute of selected product data we will update a part of the template.

            if ($('body.catalog-product-view').size() > 0) {
                // Product Page

                $("[itemprop=name]").html(selectedProductData.name);
                $("[attributeprop=description]").html(selectedProductData.description);
                var array = ['description', 'name', 'sku', 'short_description'];
                _.each(selectedProductData, function (value, code) {
                    if($.inArray(code, array) == -1) {
                        $("[attributeprop="+code+"]").html(selectedProductData[code]);
                    }
                });
